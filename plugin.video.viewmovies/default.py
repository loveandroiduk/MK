...
def GENRES(url):
        ...
        addDir2('Christmas','http://www.viewmovies.is/category/movies/comedy/',1,icon,'',fanart)
        addDir2('Comedy','http://www.viewmovies.is/category/movies/christmas/',1,icon,'',fanart)
...
def PLAYLINK(name,url,iconimage):
        link = open_url(url)
        try:
                stream_url = re.compile('src="http://videomega.tv/view.php\?ref=(.+?)"').findall(link)[0]
                stream_url = 'http://videomega.tv/view.php?ref=' +stream_url
        except:
                stream_url = re.compile('src="http://videomega.tv/validatehash.php\?hashkey=(.+?)"').findall(link)[0]

...
def CATEGORIES():
        addDir2('Recently Added','http://filmdictator.com/?filtre=date&cat=0',1,icon,'',fanart)
        addDir2('Recommended Movies','http://filmdictator.com/?filtre=random&cat=0',1,icon,'',fanart)
        addDir2('Most Popular','http://filmdictator.com/?filtre=views&cat=0',1,icon,'',fanart)
        addDir2('Movies By Category','http://filmdictator.com/categories/',4,icon,'',fanart)
...
def SEARCH():
    search_entered =''
    keyboard = xbmc.Keyboard(search_entered, 'Search Film Dictator')
    keyboard.doModal()
    if keyboard.isConfirmed():
        search_entered = keyboard.getText().replace(' ','+')
    if len(search_entered)>1:
        url = 'http://filmdictator.com/?s='+ search_entered
        link = open_url(url)
        GETMOVIES(url,name)
...
def PLAYLINK(name,url,iconimage):
        ...
        stream_url = urlresolver.HostedMediaFile(stream_url).resolve()
        print stream_url
